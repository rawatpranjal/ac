...
# ---------------------------------------------------------------------
# 0) Redirect stdout to a log file
# ---------------------------------------------------------------------
os.makedirs("exp1", exist_ok=True)
analysis_log_path = os.path.join("exp1", "analysis_stdout.txt")
orig_stdout = sys.stdout  # keep original

with open(analysis_log_path, "w") as f:
    sys.stdout = f  # all print() calls go to the file now

    # ---------------------------------------------------------------------
    # 1) Variable Definitions
    # ---------------------------------------------------------------------
    var_definitions = {
        "alpha": "Learning rate (0.05 or 0.1).",
        "gamma": "Discount factor for future rewards (0.0, 0.5, 0.9, 0.99).",
        "episodes": "Total number of training episodes (10k).",
        "reserve_price": "Reserve price in [0.0, 0.25, 0.5].",
        ...
        "avg_rev_last_1000": "Mean seller revenue in last 1000 episodes.",
        "time_to_converge": "Frac. of episodes until ±5% convergence window.",
        "avg_regret_of_seller": "Average seller regret = mean(1 - revenue).",
        "no_sale_rate": "Fraction of episodes with no valid bid (reserve unmet).",
        "price_volatility": "Std dev of winning bid across episodes.",
        "winner_entropy": "Shannon entropy of who won each episode."
    }
    ...
    # ---------------------------------------------------------------------
    df = pd.read_csv("exp1/data.csv")  # Modify path if needed

    # Normalize time_to_converge by episodes
    ...
    covariates_list = [
        'alpha', 'gamma', 'episodes', 'reserve_price',
        'init_code', 'exploration_code',
    ...
    # ---------------------------------------------------------------------
    # 5) Correlation table with treatment
    # ---------------------------------------------------------------------
    corr_rows = []
    ...
    # ---------------------------------------------------------------------
    gate_folder = os.path.join("exp1", "gate_plots")
    cate_folder = os.path.join("exp1", "cate_plots")
    os.makedirs(gate_folder, exist_ok=True)
    os.makedirs(cate_folder, exist_ok=True)

    # ---------------------------------------------------------------------
    # 7) For each outcome: (A) ATE, (B) GATE, (C) BLP CATE, (D) partial-dep
    # ---------------------------------------------------------------------
    ...
    for outcome in outcomes_list:
        ...
        if n_bin > 0:
            ...
            for i, bin_col in enumerate(bin_covs):
                ...
                ax.errorbar(xvals, eff, yerr=[eff - lo, hi - eff], fmt='o', capsize=5)
        ...
        # -----------------------------------------------------------------
        # 7D) Partial-dependence style CATE (numeric covariates)
        # -----------------------------------------------------------------
        ...
        if len(cont_covs) > 0:
            ...
            for i, cvar in enumerate(cont_covs):
                ...
                ax.plot(x_sort, eff_sort, label='CATE')
                ax.fill_between(x_sort, lo_sort, hi_sort, alpha=0.2, label='95% CI')
    ...
    # ---------------------------------------------------------------------
    # 8) Print final ATE summary across all outcomes
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # Restore stdout at the end of the with-block
    # ---------------------------------------------------------------------
    sys.stdout = orig_stdout

# Once outside the with-block, printing goes back to normal console:
